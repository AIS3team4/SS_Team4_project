...
char *make_error_resp(int* s){
	char* out;
	int len=strlen(resp_header[ERROR]);
	*s=len;
	out=(char*)malloc((len+1)*sizeof(char));
	strcpy(out,resp_header[ERROR]);
	return out;
}
	...
	if(act->action==GET){
		if(act->file_name!=NULL){
			if(strstr(act->file_name,".html")!=NULL){
				...
				total_size=strlen(resp_header[TEXT]);
				total_size+=f_size;
				output=(char*)malloc((total_size+1)*sizeof(char));
				strcpy(output,resp_header[TEXT]);
				...
				*s=total_size;
				...
				total_size=strlen(resp_header[JPEG]);
				f_size=fsize(act->file_name);
                        	total_size+=f_size;
				*s=total_size;
				output=(char*)malloc((total_size)*sizeof(char));
				strcpy(output,resp_header[JPEG]);
				
				read(fd,output+strlen(resp_header[JPEG]),f_size);
				close(fd);
			}
			else{
